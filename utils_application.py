...
def path2gt_datasets(path, dataset):
    ...
    if dataset == 'GTZAN':
        ...
        if 'grijanje' in path:
            return 0
        elif 'hladjenje' in path:
            return 1
        elif 'iskljuci' in path:
            return 2
        elif 'kuco' in path:
            return 3
        elif 'otvori' in path:
            ...
            return 5
        elif 'svjetlo' in path:
            return 6
        elif 'ukljuci' in path:
            return 7
        elif 'vrata' in path:
            return 8
        elif 'zatvori' in path:
            return 9
        else:
            print('Did not find the corresponding ground truth (' + str(path) + ')!')
    # else:
...
def prepareForClasification(word,time = 88200): # time predstavlja broj odmjeraka za 4 sekunde ponavljanja tona pri fs = 22050
    word_con = word
    for i in range(int(np.ceil((time)/len(word_con))-1)):
        word = np.concatenate((word,word_con))
    return word

def replay_words(words, fs = 22050):
    for w in words:
        sd.play(w,fs)
        sd.wait()